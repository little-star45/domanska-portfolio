...
const ProjectCard = (props) => {
  const { projectTitle, description, skills, projectName, toggleModal, setSelectedProject } = props;

  return (
    <div className="flex flex-col border-3 border-black shadow-[10px_10px_0px_0px_black] p-2 rounded-lg shadow-gray-400 transform hover:scale-105 transition duration-300 justify-between items-center w-96 h-72 text-center">
      <p className="text-xl font-bold">{projectTitle}</p>
      <p className='flex-1 overflow-y-auto'>{description}</p>
      <div>
