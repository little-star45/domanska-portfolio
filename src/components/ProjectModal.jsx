...
const ProjectModal = (props) => {
    ...
    if (markdownPath && isOpen) {
      fetch(markdownPath)
        ...
        .catch(err => ErrorEvent('Error fetching markdown:', err))
    }
  }, [markdownPath, isOpen])
    ...
    return (
        ...
        <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
            ...
            <DialogPanel className="w-full max-w-3/7 rounded-xl bg-white p-6 backdrop-blur-2xl duration-300 ease-out data-closed:transform-[scale(95%)] data-closed:opacity-0 border-2 border-black shadow-[10px_10px_0px_0px_black] space-y-4 h-contrast:bg-black h-contrast:border-yellow-300 h-contrast:shadow-[10px_10px_0px_0px_yellow] dark:bg-gray-700 dark:text-white max-h-screen">
